...
/root/go/bin/derper --hostname $TAILSCALE_DERP_HOSTNAME --bootstrap-dns-names $TAILSCALE_DERP_HOSTNAME -certmode $TAILSCALE_DERP_CERTMODE -certdir /root/derper/$TAILSCALE_DERP_HOSTNAME --a=$TAILSCALE_DERP_ADDRESS --stun --stun-port=$TAILSCALE_DERP_STUN_PORT --http-port=$TAILSCALE_DERP_HTTP_PORT --verify-clients=$TAILSCALE_DERP_VERIFY_CLIENTS
